...
public class RedisTools {
    ...
    @PostConstruct
    public void init() {
        logger.info("Init Redis Service");
        try {
            Set<HostAndPort> jedisClusterNodes = new HashSet<HostAndPort>();
            for (String server : redisServers.split(",")) {
                jedisClusterNodes.add(new HostAndPort(server.split(":")[0],
            ...
            logger.error("初始化rediscluster失败！", e);
            throw new RuntimeException("初始化rediscluster失败！", e);
        }
